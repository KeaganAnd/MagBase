...
1. **Always verify database path** - Use `-list-tables` to ensure you're working with the correct database
2. **Check table schema before inserting** - Use `-list-tables` to see column order and types
3. **Use meaningful table names** - Makes it easier to understand your database structure
4. **Plan column order carefully** - You must provide values in the exact schema order
5. **Handle NULL values explicitly** - Use `NULL` keyword for nullable fields
6. **Verify before deletion** - Use `-read-record` or `-list-records` before deleting
7. **Keep database files organized** - Store all `.mab` files in a dedicated directory
...
|-------|-------|----------|
| `Failed to open database file` | File doesn't exist or permission denied | Check file path and permissions |
| `This is not a valid MagDB` | File is corrupted or wrong format | Restore from backup or recreate |
| `Table not found` | Invalid table ID | Use `-list-tables` to find correct ID |
| `Record not found` | Invalid record ID or not in table | Use `-read-record` with correct ID |
| `Invalid number of columns` | Column count is 0 or > 16 | Specify between 1-16 columns |
| `Unknown column type` | Invalid type (not int/text/bool) | Use one of: int, text, bool |
| `Failed to create table` | Schema page allocation failed | Ensure database is not corrupted |

---

## Version Information

**MagBase Version**: 1.0.0
- Major: 1
- Minor: 0
- Patch: 0

**Page Size**: 4,096 bytes
**Max Columns Per Table**: 16
**Max Column Name Length**: 32 characters
**Max Table Name Length**: 32 characters
**Max Text Field Length**: 256 bytes
